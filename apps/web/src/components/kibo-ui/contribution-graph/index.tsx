...
export const ContributionGraph = ({
  ...
  const activityData = useMemo(() => {
    ...
    const activitiesMap = new Map<string, Activity>(
      data.map((activity) => [activity.date, activity])
    );

    return dateRange.map((date) => {
      const isoDate = formatISO(date, { representation: "date" });
      if (activitiesMap.has(isoDate)) {
        return activitiesMap.get(isoDate) as Activity;
      }
  ...
  const weeks = useMemo(
    () => groupByWeeks(activityData, weekStart),
...
export const ContributionGraphBlock = ({
  ...
  if (activity.level < 0 || activity.level > maxLevel) {
    throw new RangeError(
      `Provided activity level ${activity.level} for ${
        activity.date
      } is out of range. It must be between 0 and ${maxLevel}.`
    );
  }
  ...
  const getTooltipText = () => {
    ...
    if (count === 0) {
      return `Нет активностей за ${formattedDate}`;
    }
    ...
    return `${count} ${word} за ${formattedDate}`;
  };
...
export const ContributionGraphCalendar = ({
  ...
  return (
    <div
      ...
      className={cn(
        "max-w-full overflow-x-auto overflow-y-hidden hide-scrollbar",
        className
      )}
      {...props}
    >
